  ...
  const fetchOrders = async (page = 1) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        const processedOrders = fetchedOrders
          .map((order) => {
            try {
              ...
              if (isValidDate) {
                ...
                isExpired = validUntil.getTime() <= new Date("2025-05-11T00:00:00Z").getTime();
              }
              ...
              if (!isValidStatus || !isPremiumPlan || !isValidDate || !isExpired) {
                return null; // Skip non-expired or invalid orders
              }

              return { ...order, validUntil };
            } catch (filterError) {
              console.error("Error processing order:", order, filterError);
              return null;
            }
          })
          .filter((order) => order !== null)
          .reverse();
        console.log("Processed expired orders:", processedOrders);
        ...
        if (processedOrders.length === 0) {
          setError("No expired premium subscriptions found. Check API data or filter criteria.");
        } else {
          setError("");
        }
      } else {
                ...
                <div className="block md:hidden space-y-4">
                  {paginatedOrders.length > 0 ? (
                    ...
                    <div className="text-center text-gray-500 p-4">
                      No expired premium subscriptions found.
                    </div>
                  )}
                </div>
            ...
            <div className="grid md:grid-cols-2 gap-4 text-sm md:text-base">
              ...
              <div className="bg-gray-50 p-4 rounded-lg shadow-sm">
                ...
                {selectedOrder.validUntil
                  ? new Date(selectedOrder.validUntil).toLocaleDateString()
                  : "N/A"}
              </div>
              <div className="bg-gray-50 p-4 rounded-lg shadow-sm">
