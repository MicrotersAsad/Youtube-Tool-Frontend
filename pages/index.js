...
const Home = ({ initialBlogs = [], availableLanguages, metaUrl, meta }) => {
  ...
  const [filteredTools, setFilteredTools] = useState([]);  // Initialize with empty array
  ...
  const handleSearchChange = (e) => {
    ...
    // Filter tools based on name or description
    const filteredTools = allTools.filter((tool) => 
      tool.name.toLowerCase().includes(query) || tool.description.toLowerCase().includes(query)
    );

    // Filter blogs based on title or slug
    const filteredBlogs = processedBlogs.filter((blog) =>
      blog.translations[currentLanguage]?.title.toLowerCase().includes(query) || 
      blog.translations[currentLanguage]?.slug.toLowerCase().includes(query)
    );
    ...
    <div className="mt-4 bg-indigo-50 py-4 md:mt-8 md:py-8 xl:mt-12">
      <div className="mx-auto max-w-7xl px-4 py-4 lg:py-12">
        <div className="mx-auto my-4 max-w-2xl text-center">
          <h2 className="text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl">AI Tools That Work For You</h2>
          <p className="mt-6 text-lg leading-8 text-gray-600">Exceptionally Useful, Completely Free — No Hidden Costs.</p>
        </div>
        <div className="mb-8 flex items-center justify-between">
          {/* Left Section: Icon + Title */}
          <div className="flex items-center">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" aria-hidden="true" className="mr-3 h-8 w-8 text-yellow-500">
