 ...
 const VALID_PAYMENT_STATUSES = ['COMPLETED', 'paid', 'completed'];
  ...
  const fetchSummary = async () => {
   ...
   if (isFreePlan && generateCount >= 5) {
                  toast.error(t("Free users are limited to 5  Video Summary Generator  in their lifetime. Upgrade to premium for unlimited access."));
                  return;
                }
      ...
      <div>
        {isFreePlan ? (
          <p className="text-center p-3 alert-warning">
            {t(
              "You have {{remaining}} of 5 lifetime  Video Summary Generator  left. Upgrade to premium for unlimited access.",
          ...
          <p className="text-center p-3 alert-warning">
            {t(`Hey ${user?.username}, you have unlimited  Video Summary Generator  as a ${user.plan}  user until your subscription expires.`)}
          </p>
        )}
      </div>
      <button
