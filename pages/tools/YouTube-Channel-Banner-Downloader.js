...
const YtChannelDw = () => {
    ...
    useEffect(() => {
        const fetchContent = async () => {
            try {
                const response = await fetch(`/api/content?category=YouTube-Channel-Banner-Downloader`);
                if (!response.ok) {
    ...
    const fetchReviews = async () => {
        try {
            const response = await fetch('/api/reviews?tool=yt-channel-banner-downloader');
            const data = await response.json();
            setReviews(data);
        } catch (error) {
