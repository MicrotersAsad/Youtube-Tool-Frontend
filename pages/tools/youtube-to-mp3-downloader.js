...
const YtAudiodw =({ meta, reviews, content, relatedTools, faqs,reactions,hreflangs})   => {
...
console.log(formats);
    ...
    const fetchContent = async () => {
      try {
        ...
        const response = await fetch(
          `/api/content?category=youtube-to-mp3-downloader&language=${language}`
        );
  ...
  const handleReviewSubmit = async () => {
    ...
    try {
      const response = await fetch("/api/reviews", {
        ...
        body: JSON.stringify({
          tool: "youtube-to-mp3-downloader",
          ...newReview,
      ...
      setShowReviewForm(false);
      fetchReviews("youtube-to-mp3-downloader");
    } catch (error) {
  ...
  const handleReaction = async (action) => {
    ...
    try {
      const response = await fetch("/api/reactions", {
        ...
        body: JSON.stringify({
          category: "youtube-to-mp3-downloader",
          userId: user.email,
          action,
        }),
      });
      ...
      <div className="bg-box">
        ...
        <Head>
            ...
            <link
              ...
              href={`${meta?.url
                .toLowerCase()
                .replace("youtube-to-mp3-downloader", "youtube-to-mp3-downloader")}`}
            />
            ...
            <meta
              name="twitter:domain"
              content={meta?.url
                .replace("tools/youtube-to-mp3-downloader", "")}
            />
            <meta
            ...
            <meta name="twitter:image:alt" content="youtube-to-mp3-downloader" />
  ...
  {JSON.stringify({
    ...
    "@type": "WebPage",
    name: meta?.title,
    url: `${meta?.url}${i18n.language !== 'en' ? `/${i18n.language}` : ''}/tools/youtube-to-mp3-downloader`,
...
<Script id="faq-structured-data" type="application/ld+json">
  {JSON.stringify({
    ...
    mainEntity: faqs.map((faq) => ({
      ...
      <div className="">
        <h1 className="text-center text-white">Free YouTube To Mp3 Downloader</h1>
        <p className="text-center text-white">Convert your favorite YouTube videos into high-quality MP3 audio files instantly with our YouTube to MP3 Downloader Online. It’s fast, free, and doesn’t require any registration or login. Enjoy hassle-free access to your favorite music, podcasts, and more.</p>
        <form
...
export async function getServerSideProps(context) {
  return getContentProps("youtube-to-mp3-downloader", context.locale, context.req);
}

export default YtAudiodw;
