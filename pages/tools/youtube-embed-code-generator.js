...
const YtEmbedCode = ({ meta, reviews, content, relatedTools, faqs, reactions, hreflangs }) => {
  ...
  const FALLBACK_IMAGE = "/default-profile.png"; // Ensure this exists in public/
    ...
    const fetchContent = async () => {
      try {
        ...
        const response = await fetch(
          `/api/content?category=YouTube-Embed-Code-Generator&language=${language}`
        );
  ...
  const fetchYouTubeData = async () => {
    console.log("fetchYouTubeData triggered!");
    ...
    if (isFreePlan && generateCount >= 5) {
      toast.error(t("Free users are limited to 5 embed code generations in their lifetime. Upgrade to premium for unlimited access."));
      return;
    }
    ...
    try {
      ...
      toast.error(t("Failed to generate embed code. Please check the video URL."));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleReviewSubmit = async () => {
    ...
    try {
      ...
      // Placeholder for fetchReviews if implemented
      // await fetchReviews("YouTube-Embed-Code-Generator");
    } catch (error) {
  ...
  const handleReaction = async (action) => {
    ...
    try {
      const response = await fetch("/api/reactions", {
        ...
        body: JSON.stringify({
          category: "YouTube-Embed-Code-Generator",
          userId: user.email,
          action,
      ...
      if (action === "like") {
        if (hasLiked) {
          toast.error(t("You have already liked this."));
        } else {
          setHasLiked(true);
          setHasUnliked(false);
          toast.success(t("You liked this content."));
        }
      } else if (action === "unlike") {
        if (hasUnliked) {
          setHasUnliked(false);
          toast.success(t("You removed your dislike."));
        } else {
          setHasLiked(false);
          setHasUnliked(true);
          toast.success(t("You disliked this content."));
        }
      } else if (action === "report") {
        if (hasReported) {
          toast.error(t("You have already reported this."));
        } else {
          setHasReported(true);
          toast.success(t("You have reported this content."));
        }
      }
    } catch (error) {
  ...
  const saveChannel = () => {
    const savedChannels = JSON.parse(localStorage.getItem("savedChannels") || "[]");
    const currentTool = {
      toolName: "YouTube Embed Code Generator",
    ...
    if (existingChannelIndex === -1) {
      ...
      setIsSaved(true);
      toast.success(t("Tool saved successfully!"));
    } else {
      savedChannels.splice(existingChannelIndex, 1);
      localStorage.setItem("savedChannels", JSON.stringify(savedChannels));
      setIsSaved(false);
      toast.success(t("Tool removed from saved list."));
    }
  };
        ...
        <div className="max-w-7xl mx-auto p-4">
          <Head>
            ...
            <link
              ...
              href={`${meta?.url
                ?.toLowerCase()
                .replace("YouTube-Embed-Code-Generator", "youtube-embed-code-generator")}`}
            />
            ...
            <meta
              property="og:image"
              content="https://ytubetools.s3.eu-north-1.amazonaws.com/uploads/1732776294789-youtubeembedcodegeneratora.png"
            />
            <meta
            ...
            <meta
              name="twitter:domain"
              content={meta?.url?.replace("tools/YouTube-Embed-Code-Generator", "")}
            />
            <meta
            ...
            <meta name="twitter:image:alt" content="youtube-embed-code-generator" />
          ...
          <Script id="webpage-structured-data" type="application/ld+json">
            {JSON.stringify({
              ...
              "@type": "WebPage",
              name: meta?.title,
              url: `${meta?.url}${i18n.language !== "en" ? `/${i18n.language}` : ""}/tools/youtube-embed-code-generator`,
              description: meta?.description,
              breadcrumb: { "@id": `${meta?.url}#breadcrumb` },
              about: { "@type": "Thing", name: meta?.title },
              isPartOf: { "@type": "WebSite", url: meta?.url },
            })}
          </Script>
          ...
          <h1 className="text-3xl pt-5 text-white">{t("YouTube Embed Code Generator")}</h1>
          <p className="text-white pb-3">
            {t("YouTube Embed Code Generator is a user-friendly tool that allows you to easily create HTML embed codes for YouTube videos.")}
          </p>
          <ToastContainer />
      ...
      <div>
        {isFreePlan ? (
          <p className="text-center p-3 alert-warning">
            {t(
              "You have {{remaining}} of 5 lifetime Embed Code Generator left. Upgrade to premium for unlimited access.",
          ...
          <p className="text-center p-3 alert-warning">
            {t(`Hey ${user?.username}, you have unlimited Embed Code Generator as a ${user.plan}  user until your subscription expires.`)}
          </p>
        )}
      </div>
      <button
            ...
            <div>
              <div className="w-full p-6">
                <label className="block text-gray-700 text-sm font-bold mb-2 text-left">
                  {t("Enter YouTube Video URL")} <span className="text-red-500">*</span>
                </label>
                <input
                  type="text"
                  className="w-full p-2 border border-gray-300 rounded-md"
                  placeholder={t("e.g. www.youtube.com/watch?v=xxxxxxx or youtu.be/xxxxxxx")}
              ...
              <div className="flex items-center mb-2">
                ...
                <div className="flex">
                  {[...Array(5)].map((_, i) => (
                    <FaStar
                      key={i}
                      color={i < Math.round(parseFloat(overallRating)) ? "#ffc107" : "#e4e5e9"}
                    />
                  ))}
                </div>
                <div className="ml-2 text-sm text-gray-500">
                  {reviews?.length || 0} {t("global ratings")}
                </div>
              </div>
              <div>
              ...
              {reviews?.slice(0, 5).map((review, index) => (
                <div key={index} className="border p-6 m-5 bg-white">
                  <div className="flex items-center mb-4">
                    <Image
                      src={review?.userProfile && review.userProfile !== "not available" ? review.userProfile : FALLBACK_IMAGE}
                  ...
                  <div className="flex items-center">
                    ...
                    <div>
                      <span className="fw-bold mt-2 ms-2">{review?.title || "Review"}</span>
                    </div>
                  </div>
              ...
              {showAllReviews &&
                reviews?.slice(5).map((review, index) => (
                  <div key={index} className="border p-6 m-5 bg-white">
                    ...
                    <div className="text-lg font-semibold">{review.title || "Review"}</div>
                    <div className="text-lg mb-4">{review.comment}</div>
                    <div className="flex items-center">
                      {[...Array(5)].map((_, i) => (
                        <FaStar
...
export async function getServerSideProps(context) {
  return getContentProps("YouTube-Embed-Code-Generator", context.locale, context.req);
}
