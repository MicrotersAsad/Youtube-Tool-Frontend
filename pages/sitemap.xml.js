...
//         return { props: {} };
//       }
//   console.log(youtubeArticles.data);
...
export async function getServerSideProps({ req, res }) {
  ...
  try {
    ...
    console.log('YouTube Articles:', youtubeArticles.data);

    // Initialize counters for total articles and articles per language
    let totalArticles = 0;
    let languageCounts = {};

    // Iterate over the articles to count them
    youtubeArticles.data.forEach(article => {
      totalArticles++;  // Increment total count
    ...
    console.log('Articles per Language:', languageCounts);
